...
external gen_keypair_ : public key -> secret key -> int = "caml_crypto_gen_keypair"

let generate () =
  let public : public key = Bytes.create public_key_length in
  let secret : secret key = Bytes.create secret_key_length in
  if gen_keypair_ public secret < 0
  then Or_error.error_s [%message "failed to generate ed25519 keypair"]
  else Or_error.return { secret; public }
;;

external dh_ : shared key -> secret key -> public key -> int = "caml_crypto_dh"

let dh ~(public : public key) ~(secret : secret key) =
  let shared = Bytes.create shared_key_length in
  if dh_ shared secret public < 0
  then Or_error.error_s [%message "failed to do ecdh with provided key material"]
