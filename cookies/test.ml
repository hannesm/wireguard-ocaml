...
let%expect_test "test_cookie_mac1_check_from_go" =
  let key = Crypto.generate () in
  let%bind check = Checker.init key.public in
  let%bind () =
    let msg = checkMAC1([]byte{
...
let%expect_test "test_cookie_reply_from_go" =
  ...
  let%bind cookie_reply = Checker.create_reply ~t:check ~msg ~receiver:(Int32.of_int 1377) ~src in
...
let check_mac2 ~gen ~check ~msg =
  ...
  Messages.xor_dummy msg '\x20';
  let%bind () = Checker.check_macs ~t:check ~msg ~src |> is_error in
  Messages.xor_dummy msg '\x20';
  let src_bad1 = int_array_to_bytes {192, 168, 13, 37, 40, 01} in
  let%bind () = Checker.check_macs ~t:check ~msg ~src:src_bad1 |> is_error in
  let src_bad2 = int_array_to_bytes {192, 168, 13, 38, 40, 01} in
...
let%expect_test "test_cookie_mac2_check_from_go" =
  ...
  in
  let msg = [0x0e, 0x2f, 0x0e, 0xa9, 0x29, 0x03, 0xe1, 0xf3,
		...
		0x7d, 0xa1, 0xd5, 0x85, 0x6d, 0xf0, 0x1b, 0xaa] in
  check_mac2 ~gen ~check ~msg;
  [%expect {| |}]
;;
