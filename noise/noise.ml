...
(* let construction = Bytes.of_string
   "Noise_IKpsk2_25519_ChaChaPoly_BLAKE2s" let identifier = Bytes.of_string
...
let second_message_of_fields ~msg_sender ~msg_receiver ~msg_ephemeral
    ~msg_empty =
  assert (Bytes.length msg_sender = 4) ;
...
let first_message ~(msg_sender : bytes) ~(timestamp : bytes)
    ...
    ~(s_r_public : Nacl_crypto.Key.public Nacl_crypto.Key.key)
    ~(s_i : Nacl_crypto.Key.keypair) : ((bytes * bytes) * bytes) Or_error.t
    =
...
let%expect_test "test-handshake-no-macs" =
  ...
  in
  let e_r = Nacl_crypto.Key.{secret= e_r_private; public= e_r_public} in
  let s_r_public =
    Bytes.of_string
  ...
  in
  let s_r = Nacl_crypto.Key.{secret= s_r_private; public= s_r_public} in
  let e_i_public =
    Bytes.of_string
  ...
  in
  let e_i = Nacl_crypto.Key.{secret= e_i_private; public= e_i_public} in
  let s_i_public =
    Bytes.of_string
